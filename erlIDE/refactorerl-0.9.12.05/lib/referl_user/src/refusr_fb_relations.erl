...
%% functionblocks.
is_relation([])->
    throw({error, "No functionblock list was given!~n", []});
is_relation([{_, _} = Arg | []])->
    is_relation(tuple_to_list(Arg));
is_relation({_,_} = Arg)->
    is_relation(tuple_to_list(Arg));
is_relation(Arg) when not is_list(Arg) and not is_tuple(Arg)->
    throw({error, "Error, the form of the arguments was not proper. 
		It should be given in a list or a in a tuple! ~n", []});
...
is_relation(_) -> throw({error, 
			"The number the functionblocks was not good. It should be exactly two.~n", []}).
...
check_cycle([{_, _} | _ ] = Options)-> %% call from ri 
   ?LibRel:invest_table(fb_rel, [{type, check} |Options]);
check_cycle(ParList)->
   ?LibRel:invest_table(fb_rel, [{type, check}, {gnode, ParList}]).
...
draw(PropList)->
    ?LibRel:invest_table(fb_rel, [{type, draw} | PropList]).
...
draw_cycles(PropList)->
     ?LibRel:invest_table(fb_rel, [{type, cycles} | PropList]).
