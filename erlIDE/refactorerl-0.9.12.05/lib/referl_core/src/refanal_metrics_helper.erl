...
-define(Tab, metric_graph).
-define(MNodes, metric_nodelist).
-define(MTransform, metric_transform).
-define(Mode, "metricmode.dat").
-define(Defs, "metricmode.defs").
...
tokenpos(Node, File) ->
    case ?Graph:path(Node, [{fundef,back},{funcl, 1}, name, {elex, 1}]) of
        [Token] -> reflib_token:pos(File, Token);
        []      -> {0, 0}
    end.
...
tab2list(Tab) ->
     Level1 = 
     lists:flatten([mnesia:dirty_read({Tab, Key}) || 
            Key <- mnesia:dirty_all_keys(Tab)]),
      [{Key, Value}|| {_RName, Key, Value}<- Level1].
...
init(State) ->
    ...
    case ?Db:exists(?Tab) of
           false -> ?Db:create(?Tab, 
            record_info(fields, metric_graph),[{type, set}]);
	   _ -> ok
    end,
    case ?Db:exists(?MNodes) of
           false -> ?Db:create(?MNodes, record_info(fields, metric_nodelist),
                    [{type, set}]);
	   _ -> ok
    end,
    case ?Db:exists(?MTransform) of
           false ->  ?Db:create(?MTransform, record_info(fields, metric_transform), 
                     [{type, set}]);
	   _ -> ok
    end,
    mnesia:wait_for_tables([?Tab, ?MNodes, ?MTransform], infinity),
    mes(State).
...
metric_process(Mod, State)->
    ...
    [begin
        insert_into_db(
        ?Tab, {Fun, {[], function, [unknown], inactive}}),
        {Fun, {[], function, [unknown], inactive}}
     end
    || Fun <- FunsS],
...
add_mods(Modules) ->
    [insert_into_db(
       ?Tab, {Mod, {[], module, [unknown], inactive}})
                                           || Mod <- Modules].
...
insert_into_db(Tab, {Key, Data}) ->
    mnesia:dirty_write({Tab, Key, Data}).
