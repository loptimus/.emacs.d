...
function_calls(Module) ->
    ?Query:exec(Module, ?Query:seq([?Mod:locals(), ?Fun:definition(), ?Form:clauses(), ?Clause:subscopes(),
    								?Clause:exprs(), ?Expr:top_sub(), ?Expr:function()])).

%    ?Query:all([func, {fundef, back}, funcl, {scope, back}, visib, {top, back}, funeref],
%   			[func, {fundef, back}, funcl, {scope, back}, visib, {top, back}, funlref])).
