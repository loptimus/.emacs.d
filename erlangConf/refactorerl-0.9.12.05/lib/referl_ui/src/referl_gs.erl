...
%%% @doc Erlang GS user interface management functions.
%%%
...
set_sizes() ->
    Width = 800,
    Height = 600,
	gs:config(win, [{width,800},{height,600}]),
...
tr_ok(Tr) ->
    ...
    case Tr#tr.label of
        "Run query" -> [{root, gs:read(tree, data)}];
        _ -> []
    end,
    gs:destroy(trwin),
...
do_transform(transform, Fun, Args) ->
    ...
    receive
        {ReqID, reply, R} ->
            case R of
                {error, {_Code, Msg}} ->
                    errorwindow("Error: " ++ Msg),
                    error;
                {ok, {abort,{_Code, Msg}}} ->
                    errorwindow("Abort: " ++ Msg);
                {ok, {result, [{result, Queryres}]}} ->
                    querywindow(Queryres);
                _ ->
                    R
            end
	end.
...
errorwindow(ERROR) ->
    ...
    gs:create(window, errorwin, win,
	      [{x, H1}, {y, 300}, {height, 150}, {width, L*7}, 
	       {map, true}, {title, "ERROR"}]),
    gs:create(label, errorwin,
	      [{x, 0}, {y, 0}, {width, L*7}, {height, 100},
...
querywindow(Querystr) ->
    gs:create(window, querywin, win, 
        [{height, 300},{width, 400},{map, true},{title, "Query result:"}]),
    gs:create(editor, queryedit, querywin, 
        [{enable, true},{wrap, none},{vscroll, bottom},{hscroll, right},
         {x,10},{y,10},{width,380},{height,250}]),
    gs:create(button, querywin, [{label, {text, "OK"}},
        {data, querybutton},{x,10},{y,265}]),
    gs:config(queryedit, {insert, {insert, Querystr}}),
    gs:config(queryedit, {enable, false}),
    QueryLines = string:tokens(Querystr, "\n"),
    LineNum = length(QueryLines),
    MaxLineLength = lists:max([length(X) || X<-QueryLines]),
    makescroll(queryedit, {LineNum, 16}, {MaxLineLength, 50}).
