...
-export([backup/0, restore/1, ls_backups/0, undo/0, redo/0, clean/0]).
...
restore(Count) ->
    case restore_checkpoint(Count) of
        {atomic, _} -> ok;
        Else -> Else % fixme: error handling?
    end.
...
list_checkpoints() ->
    {ok, [name(?NAME, N) || N <- lists:seq(1, checkpoint_count())]}. % fixme: lists:seq(1,0) is possible

checkpoint()->
    %%temporary function call
    delete_all_backups(1),
    Name  = name(?NAME, 1),%checkpoint_count()),
