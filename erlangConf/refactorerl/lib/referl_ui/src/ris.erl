...
%% If no error was detected, saves the database.
with_db_save(Result) ->
    result_is_ok(Result) andalso
        ?Graph:save(database),
    Result.
...
add({Filename=[C|_]}) when is_integer(C)->
    with_db_save(add_byname(Filename));
add(Source)->
    with_db_save(add_drop(add_dir,Source)).

drop(Source)->
    with_db_save(add_drop(drop_dir,Source)).
