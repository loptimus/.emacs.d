...
undo :: State -> String -> Int -> IO (State, [String])
undo s fn n = do 
	(buffers', fs) <- undo0 (buffers s) fn n (refactorings s) 
 	let s' = s { buffers = buffers' }
	return (s', fs)
	
undo0 :: Buffers -> String -> Int -> Refactorings -> IO (Buffers, [String])
undo0 bs@(Buffers xs ys) fn 0 rs@(Refactorings ors) = do
	let ([x], xs'') = L.partition (\b -> fileName b == fn) xs
	let cId = lastUId (undos x)
	if IM.member cId ors
		  then do 
		  	(bs0, ids0) <- undoUntilC fn (S.empty) [cId] bs rs
...
undo0 bs@(Buffers xs ys) fn n rs@(Refactorings ors) = do
	let ([x], xs'') = L.partition (\b -> fileName b == fn) xs
	let cId = lastUId (undos x)
	...
	if not $ L.null cIds && (L.length cIds > 1 || IM.member (head cIds) ors || L.head cIds /= cId)
		  then do 
		  	(bs0, ids0) <- undoUntilC fn (S.empty) cIds bs rs
			let cfs = map fileName xs
		  	let fs0 = L.foldl L.union [] $ mapMaybe (`IM.lookup` ors) ids0
			--let ufs = map (\x -> (x, lastUndoNames x 5)) fs0
			--let ufs = L.map fileName $ L.filter (L.null . getUndos . undos) $ openedBufs bs0
...
undoUntilC fn cs [] bs@(Buffers xs ys) rs@(Refactorings ors) = 
	return (bs, S.toList cs)
undoUntilC fn cs (i:is) bs@(Buffers xs ys) rs@(Refactorings ors) = do
	...
	bs' <- if i `S.member` cs
		then undoUntilC fn cs is bs rs
		else case IM.lookup i ors of
			Nothing -> undoUntilC fn (i `S.insert` cs) (is ++ is') bs' rs
			 where 
			 	bs' = bs { openedBufs = xs'}
				xs' = xs0' : xs1
			...
			Just fs -> do 
				...
				case (unzip $ map undoUntilB' xs0, unzip ys0'') of 
					((xs0', is1), (ys0', is2)) -> undoUntilC fn (i `S.insert` cs) (is ++ (concat $ is1 ++ is2)) (Buffers (xs1 ++ xs0') (ys1 `M.union` (M.fromList $ map (\x -> (fileName x,x)) ys0'))) rs
	  		 where 
...
redo :: State -> String -> Int -> IO (State, [String])
redo s fn n = do
	(buffers', fs) <- redo0 (buffers s) fn n (refactorings s)
	let s' = s { buffers = buffers' }
	return (s', fs)
...
redo0 :: Buffers -> String -> Int -> Refactorings -> IO (Buffers, [String])
redo0 bs@(Buffers xs ys) fn 0 rs@(Refactorings ors) = do
	let ([x], xs'') = L.partition ((== fn) . fileName) xs
	let cId = lastRId $ undos x
 	if IM.member cId ors
	  then do 
	  	(bs0, ids0) <- redoUntilC fn (S.empty) [cId] bs rs
		let cfs = map fileName xs
		let fs0 = L.foldl L.union [] $ mapMaybe (`IM.lookup` ors) ids0
		...
		let rfs = L.map fileName $ L.filter (L.null . getRedos . undos) $ openedBufs bs'
	  	return (bs', [fileName x])
redo0 bs@(Buffers xs ys) fn n rs@(Refactorings ors) = do
	let ([x], xs'') = L.partition ((== fn) . fileName) xs
	let cId = lastRId (undos x)
	...
	--putStrLn $ show cIds
 	if not $ L.null cIds && (L.length cIds > 1 || IM.member (head cIds) ors || L.head cIds /= cId)
	  then do 
	  	(bs0, ids0) <- redoUntilC fn (S.empty) cIds bs rs
		let cfs = map fileName xs
		let fs0 = L.foldl L.union [] $ mapMaybe (`IM.lookup` ors) ids0
		let fs = L.nub $ fn : fs0
		--let rfs = L.map fileName $ L.filter (L.null . getRedos . undos) $ openedBufs bs0
...
redoUntilC fn cs [] bs@(Buffers xs ys) rs@(Refactorings ors) = do
  return (bs, S.toList cs)
redoUntilC fn cs (i:is) bs@(Buffers xs ys) rs@(Refactorings ors) = do
	let redoUntilB' buffer = redoUntilB buffer i
	let redoUntilCB' buffer = redoUntilClosedB buffer i
	bs' <- if i `S.member` cs 
		then redoUntilC fn cs is bs rs
		else case IM.lookup i ors of
			Nothing -> redoUntilC fn (i `S.insert` cs) (is ++ is') bs' rs
			 where
			 	bs' = bs { openedBufs = xs'}
				xs' = xs0' : xs1
			...
			Just fs -> do
				...
				case (unzip $ map redoUntilB' xs0, unzip ys0'') of 
					((xs0', is1),(ys0',is2)) -> redoUntilC fn (i `S.insert` cs) (is ++ (concat $ is1 ++ is2 )) (Buffers (xs1 ++ xs0') (ys1 `M.union` (M.fromList $ map (\x -> (fileName x,x)) ys0'))) rs
			 where
			  (xs0, xs1) = L.partition (\b -> any (\f -> f == fileName b) fs ) xs
			  (ys0, ys1) = M.partition (\b -> any (\f -> f == fileName b) fs ) ys
	return bs'
