...
create_modlist([First|Tail]) ->
    ...
    if
        ...
        is_list(First) ->
            [{First,Mods}] = refusr_fb_regexp:re([{type, list},{regexp, [First]}]),
            Mods;
        true ->
            io:format("Error: not a valid module or layer definition: ~p.~n",[[First|Tail]]), 
            throw({error, not_valid_moddef})
    end,
