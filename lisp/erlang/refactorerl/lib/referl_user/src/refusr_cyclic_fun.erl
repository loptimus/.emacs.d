...
check_function([{_, _} | _ ] = Options)-> %% call from ri 
   output(?LibRel:invest_table(func, Options));
check_function(Function)->
   output(?LibRel:invest_table(func, [{type, check}, {gnode, Function}])).
...
print_cycle([{_, _} | _ ] = Options)-> 
   ?LibRel:invest_table(func, Options);
print_cycle(FromNode)->
   ?LibRel:invest_table(func, [{type, print}, {gnode, FromNode}]).
...
show_result(Graph, Routes, _)->
    ...
    throw({error, "Invalid type command given, as a default, routes listed above.~n ", []}).
...
%% '''
draw_cycles(Options)->
     ?LibRel:invest_table(func,[{type, cycles} | Options]).
...
funcalls({_, Type, _}) when Type /= func->
	throw({error, "Error, bad argument given, a function type gnode should be the argument!"});
funcalls(Fun)->
	[diff_edge_calls(Fun, [funcall, dyncall, ambcall, may_be], [])].
...
%%% Building a digraph
build_graph(Graph, Exceptions, Leaves)->
    AllFuns = get_funcall(get_all_functions()),
...
build_vertex(Graph, [{Fun, Callees} | Rest], {Exceptions, Otp}, Leaves) ->
	...
	Bool = lists:member(Fun, Exceptions) or is_otp_member(Fun, Otp),
	NewExceptions = case Bool of
			true -> 
				Exceptions++Callees;
			false -> check_subgraph(Graph, Fun),
    	...
    	build_vertex(Graph, Rest, {NewExceptions, Otp}, Leaves).
...
build_subgraph(Graph, Node, Exceptions, Leaves)->
	...
	[{Fun, CalledList}]->  
		...
		NewVertices -> NewExceptions = build_vertex(Graph, [{Fun, CalledList}], Exceptions, Leaves),
