...
erlang_rules() ->
    ...
    %#region % white spaces and required line breakers
     [
        ...
        % Unary operator
        #tokenRule{
            parents = [{expr, prefix_bit_expr},{expr, prefix_expr},{lex,body}],
            rules   = [{['+','-'],                   [{ws,1,0}]},
                       {['not','bnot'],              [{ws,1,1}]} ]},
        #tokenRule{
            parents = [{expr, catch_expr},{lex,body}],
            rules   = [{['catch'],            [{ws,0,1}]} ]},
        % Binary operator
        #tokenRule{
            parents = [{expr, infix_expr},{lex,body}],
            rules = [
                {['+','-','*','/','++','--'], [{ws,1,1}]},
        ...
        % Expression
        #tokenRule{
            parents = [{expr, block_expr}, {lex,body}],
            rules   = [{['begin'],    [{ws,0,1},{nl,0,1}]},
                       {['end'],      [{ws,1,0},{nl,1,0}]} ]},
        #tokenRule{
            parents = [{expr, if_expr}, {lex, body}],
            rules   = [{['if'],       [{ws,0,1},{nl,0,1}]},
                       {[';'],        [{ws,0,1},{nl,0,1}]},
                       {['end'],      [{ws,1,0},{nl,1,0}]} ]},
        #tokenRule{
            parents = [{expr, case_expr}, {lex, body}],
            rules   = [{['case'],            [{ws,0,1}]},
                       {['of'],              [{ws,1,1},{nl,0,1}]},
                       {[';'],               [{ws,0,1},{nl,0,1}]},
                       {['end'],             [{ws,1,0},{nl,1,0}]} ]},
        #tokenRule{
            parents = [{expr, receive_expr}, {lex, body}],
            rules   = [{['receive'],    [{ws,0,1},{nl,0,1}]},
                       {[';'],          [{ws,0,1},{nl,0,1}]},
                       {['after'],      [{ws,1,1},{nl,1,1}]},
                       {['end'],        [{ws,1,0},{nl,1,0}]} ]},
        #tokenRule{
            parents = [{expr, try_expr}, {lex, body}],
            rules   = [{['try'],                    [{ws,0,1},{nl,0,1}]},
                       {[';'],                      [{ws,0,1},{nl,0,1}]},
                       {['of','catch','after'],     [{ws,1,1},{nl,1,1}]},
                       {['end'],                    [{ws,1,0},{nl,1,0}]} ]},
        #tokenRule{
            parents = [{expr, fun_expr}, {lex, body}],
            rules   = [{['fun'],             [{ws,0,0}]},
                       {['end'],             [{ws,1,0}]}]},
        #tokenRule{
            parents = [{expr, implicit_fun}, {lex, body}],
            rules   = [{['fun'],             [{ws,0,1}]}]},
        #tokenRule{
        ...
        #tokenRule{
            parents = [{expr, match_expr}, {lex, body}],
            rules   = [{['='],                        [{ws,1,1}]}]},
        #tokenRule{
            parents = [expr],
    ...
    %#region % terms in multiple lines
     [
        ...
        #tokenRule{
            parents = [{clause,fundef},{clause, funexpr},{clause,guard},
                       {clause,timeout},{clause,pattern}, {lex, body}],
            rules   = [{['->'],        [{nl,0,1}]}]},
        #tokenRule{
            parents = [{clause, funexpr}, {lex, body}],
