...
-export([run_cluster/0, run_cluster/1, run_cluster_default/0,
         ...
         run_cluster_check_fun/0, run_cluster_check_fun/1,
...
run_cluster_check_fun() ->
    lists:append([run_cluster_check_fun(alg),
                  run_cluster_check_fun(all),
                  run_cluster_check_fun(agglom_attr),
                  run_cluster_check_fun(genetic),
                  run_cluster_check_fun(kmeans)]).

run_cluster_check_fun(alg) ->
    [{alg, undefined}];
run_cluster_check_fun(all) ->
    [{modules, undefined},
     {functions, undefined},
     {log_output, undefined},
     {print_options, undefined},
     {entities, undefined}];
run_cluster_check_fun(agglom_attr) ->
    [{skip_modules, fun check_optionallist/1},
     {skip_functions, fun check_optionallist/1},
     {transformfun, fun check_transformfun/1},
     {distfun, fun check_distfun/1},
     {anti_gravity, fun check_probability/1},
     {mergefun, fun check_mergefun/1}];
run_cluster_check_fun(genetic) ->
    [{population_size, fun check_integer/1},
     {iterations, fun check_integer/1},
     {mutation_rate, fun check_probability/1},
     {crossover_rate, fun check_probability/1},
     {elite_count, fun check_integer/1},
     {max_cluster_size, fun check_integer/1},
     {max_start_cluster_size, fun check_integer/1}];
run_cluster_check_fun(kmeans) ->
    [{stoppingcriteria, undefined}, %%TODO
     {mergefun, undefined}, %%TODO
     {distfun, undefined}, %%TODO
     {anti_gravity, fun check_probability/1},
     {k, fun check_integer/1}]. %%TODO    

check_optionallist(String) ->
    case String of
        ""     -> {false, undefined};
        "none" -> {true, undefined};
        _      ->
            Words = string:tokens(String, " ,"),
            Entities = [list_to_atom(Word) || Word <- Words],
            {true, Entities}
    end.
